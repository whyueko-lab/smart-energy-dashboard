...
# Grafik Per Jam berdasarkan prediksi realistis
st.markdown("### 🕒 **Total Penggunaan Energi Tiap Jam (Prediksi Realistis per Jam)**")

jam_list = list(range(24))
prediksi_data = []

for jam in jam_list:
    # Suhu berdasarkan jam
    if 0 <= jam < 6:
    ...
    # Penghuni berdasarkan jam
    penghuni = 1 if (5 <= jam <= 7 or 18 <= jam <= 23) else 0

    # Cahaya berdasarkan jam
    if 6 <= jam <= 17:
        cahaya = random.uniform(70, 100)
    else:
        cahaya = random.uniform(0, 30)

    # Cuaca acak
    cuaca = random.choice(["cerah", "mendung", "hujan"])
    hari_libur = 0  # tetap diasumsikan bukan hari libur
    ...
    daya_lain = 50  # Daya perangkat lain yang tetap menyala
    daya_total = daya_ac + daya_tv + daya_lampu + daya_lain
    daya_total = round(daya_total, 1)
...
# Visualisasi dengan warna berdasarkan suhu
