...
# Tombol untuk reset log
if st.button("🗑️ Reset Log Data"):
    st.session_state["log"] = []  # kosongkan log dari session
    if os.path.exists(LOG_FILE):
        os.remove(LOG_FILE)  # hapus file CSV log
...
tv_state = st.radio("Status TV", ["OFF", "ON"], index=0, horizontal=True, key="tv_state")
if tv_state == "ON":
    st.success("TV Dinyalakan")
else:
    st.warning("TV Dimatikan")
...
daya_tv = 100 if tv_state == "ON" else 0  
...
# Buat kolom jam bulat
df_log["Waktu Jam"] = pd.to_datetime(df_log["Waktu"]).dt.floor("H")

# Ambil hanya 1 entri per jam (misalnya yang terakhir)
df_log_per_jam = df_log.drop_duplicates(subset=["Waktu Jam"], keep="last")

# Tampilkan log yang diringkas per jam
st.dataframe(df_log_per_jam.drop(columns=["Waktu Jam"]), use_container_width=True)
