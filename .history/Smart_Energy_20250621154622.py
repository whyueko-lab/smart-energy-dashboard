...
# Grafik Per Jam
# Grafik Per Jam berdasarkan log (Total Energi)
st.markdown("### 🕒 **Total Penggunaan Energi Tiap Jam (Log)**")

# Pastikan waktu dalam format datetime
df_log["Waktu"] = pd.to_datetime(df_log["Waktu"])

# Ambil jam dari waktu
df_log["Jam"] = df_log["Waktu"].dt.hour

# Totalkan daya per jam
total_daya_per_jam = df_log.groupby("Jam")["Daya Total (W)"].sum().reset_index()

# Plot grafik
fig_total_jam = px.bar(
    total_daya_per_jam,
    x="Jam",
    y="Daya Total (W)",
    text="Daya Total (W)",
    labels={"Jam": "Jam (0-23)", "Daya Total (W)": "Total Energi (Watt)"},
    title="Total Penggunaan Energi per Jam (Berdasarkan Log)"
)
fig_total_jam.update_traces(textposition="outside")
fig_total_jam.update_layout(xaxis=dict(dtick=1))
st.plotly_chart(fig_total_jam, use_container_width=True)
