...
# Grafik Per Jam
# Grafik Per Jam (berdasarkan log nyata)
st.markdown("### 🕒 **Rata-rata Energi Tiap Jam (Berdasarkan Log)**")
...
# Hitung rata-rata energi tiap jam
rata_jam = df_log.groupby("Jam")["Daya Total (W)"].mean().reset_index()
rata_jam.columns = ["Jam", "Rata-rata Energi (W)"]

# Plot
fig_jam_real = px.line(rata_jam, x="Jam", y="Rata-rata Energi (W)", markers=True, title="Rata-rata Energi Tiap Jam (Log)")
fig_jam_real.update_layout(xaxis=dict(dtick=1))
st.plotly_chart(fig_jam_real, use_container_width=True)
