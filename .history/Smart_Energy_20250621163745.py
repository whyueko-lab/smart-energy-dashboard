...
def train_ml_model():
    ...
    # Split data
    X_train_ac, X_test_ac, y_train_ac, y_test_ac = train_test_split(X, y_ac, test_size=0.2, random_state=42)
    X_train_tv, X_test_tv, y_train_tv, y_test_tv = train_test_split(X, y_tv, test_size=0.2, random_state=42)
    X_train_lampu, X_test_lampu, y_train_lampu, y_test_lampu = train_test_split(X, y_lampu, test_size=0.2, random_state=42)

    # Train models
    model_ac = Pipeline([("pre", transformer), ("clf", RandomForestClassifier())]).fit(X_train_ac, y_train_ac)
    model_tv = Pipeline([("pre", transformer), ("clf", RandomForestClassifier())]).fit(X_train_tv, y_train_tv)
    model_lampu = Pipeline([("pre", transformer), ("clf", RandomForestClassifier())]).fit(X_train_lampu, y_train_lampu)


    # Evaluation
    acc_ac = accuracy_score(y_test_ac, model_ac.predict(X_test_ac))
    acc_tv = accuracy_score(y_test_tv, model_tv.predict(X_test_tv))
    acc_lampu = accuracy_score(y_test_lampu, model_lampu.predict(X_test_lampu))


    # Simpan skor ke session_state untuk dipakai di sidebar
    st.session_state["acc_ac"] = acc_ac
    st.session_state["acc_tv"] = acc_tv
    st.session_state["acc_lampu"] = acc_lampu
...
st.sidebar.metric("AC", f"{st.session_state.get('acc_ac', 0)*100:.2f}%")
st.sidebar.metric("TV", f"{st.session_state.get('acc_tv', 0)*100:.2f}%")
st.sidebar.metric("Lampu", f"{st.session_state.get('acc_lampu', 0)*100:.2f}%")
