...
# Total Penggunaan Energi Berdasarkan Log (Keseluruhan)
st.markdown("### 🔋 **Total Penggunaan Energi Berdasarkan Log (Keseluruhan)**")

# Pastikan waktu datetime
df_log["Waktu"] = pd.to_datetime(df_log["Waktu"])

# Hitung total konsumsi energi dari seluruh log
total_konsumsi = df_log["Daya Total (W)"].sum()

# Buat DataFrame satu bar
df_total = pd.DataFrame({
    "Kategori": ["Total Energi"],
    "Daya Total (W)": [total_konsumsi]
})
...
fig_total = px.bar(
    ...
    y="Daya Total (W)",
    text="Daya Total (W)",
    title="Total Energi yang Digunakan (Berdasarkan Log)"
)
